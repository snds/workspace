...
name: reconcile
description: >
  Merges Session Blocks from multiple concurrent sessions (different machines, same day) into
...
> Canonical home for every surface. The `.claude/skills/reconcile/` and `.agents/skills/reconcile/` copies (and the snds plugin's) are generated pointer wrappers (`09-tools/build-local-skill-plugin.py`); edit this file, never a wrapper.
