...
| `wsx scan [--json] [--write]` | Detect the user's **own** stack so the interview pre-fills instead of asking cold: installed agents (PATH + config dirs), configured **MCP servers** (names only — never reads keys/env), and **local LLMs** (localhost probes: Ollama/LM Studio/Jan). Suggests surfaces/models. Reinforces BYO-tokens — the engine has no API key and makes no model calls. Works with or without a workspace. | **built ✓** |
