...
| `wsx project new <name>` \| `list` | Per-project **documentation** folders (docs & context, **not** the code/assets). `new` scaffolds `projects/<slug>/PROJECT.md` (overview · where the code lives · live handoff · decisions · pending) + `notes/`, and regenerates the projects index + HOME. `.gitignore` guards keep code trees (node_modules, dist, .git…) out of the vault. | **built ✓** |
