...
> ⚠️ **Status: early, but runnable.** The design is settled (see [SPEC.md](SPEC.md), v0.2) and a first cut of the `wsx` CLI (**v0.1**) is built and passes a smoke test: `init`, `profile get`/`set`, `emit` (`claude-code` · `agents-md` · `cursor` · `pack` · `all`), `lint`, `verify`, `session`, and `sync` all run today. `resolve` and the `mcp` emit target are **honest stubs** (they print what they would do and exit cleanly). The interview/synthesis/resolver *brain* exists as markdown but is not yet wired into an automated loop. Trust the command's own `--help` and the code over this prose if they ever disagree.
...
| `wsx resolve` | The mechanical half of the Resolver: fetch a pulled skill from a registry, **pin** it to a version, and **namespace** it. The brain makes the pull/adapt/generate *decision*; `wsx resolve` executes the fetch+pin. | **stub** |
...
| `wsx skill add` \| `list` \| `reindex` | The mechanical hand for the Resolver's **GENERATE** path: create a skill folder + front matter and register it in `manifest.json` (`add`), list skills grouped by hub (`list`), or rebuild the manifest index from disk (`reindex`). | **built ✓** |
...
*The build is early but real: `init`, `profile`, `emit` (`claude-code`/`agents-md`/`cursor`/`pack`/`all`), `lint`, `verify`, `session`, and `sync` run today and pass a smoke test; `resolve` and the `mcp` target are honest stubs; the interview brain exists as markdown but isn't yet wired into an automated loop. When the code and this doc disagree, the code and its `--help` are the truth; this doc is the map.*
