...
| `wsx search [--kind skill\|reference] [--source id]` | The discovery layer feeding two-track sourcing. Reads a pluggable source catalog (`context/sources.json`, else a built-in default). For **skill-registry** sources with a JSON `index_url`: fetch → filter by query → print candidates + a ready-to-paste plan fragment. Without an index: point at the homepage (unvetted sources flagged). For **reference** sources: list anchors + the honest note that reference discovery is the brain's own research. | **built ✓** |
| `wsx resolve [--plan F] [--update] [--allow-unvetted] [--cache-refs]` | The mechanical half of the Resolver. Reads an approved `context/skill-plan.json` and, per entry: **PULL** = fetch (`http(s)`/`file`/local) → **pin** (byte-identical, `0o444` read-only) → **namespace** (`skills/pulled-<registry>-<name>/`) → register; **PULL+PATCH** = pull + editable sibling `overlay.md` (composed into the emitted Claude-Code skill); **GENERATE** = delegate to `skill add`; **COMPOSITE** = generate + cite `references[]` (author-voice `## Sources` block; `--cache-refs` fetches+pins each reference). Enforces the trust gate (unvetted refused unless `audited`), pin-drift safety (`--update`), and idempotency. The brain decides; `wsx resolve` executes. | **built ✓** |
| `wsx emit <target>` | Compile the canonical workspace (`context/` + `skills/` + `manifest.json`) into a surface-specific adapter. Targets: `claude-code` \| `agents-md` \| `cursor` \| `mcp` \| `pack` \| `all`. | **built ✓** |
...
| `wsx health` | Vault **graph hygiene** (input-time twin of lint): **orphan nodes** (notes nothing links to), **stale claims** (`#stale` tags + `as of YYYY-MM` dates past a 12-month horizon), and **dangling typed edges** (`relations:` pointing at a missing note). Orphans/aging are advisory; `#stale` + dangling set the exit code so a scheduled run can gate. Zero-dep. | **built ✓** |
...
| `wsx session start` \| `end` \| `reconcile` | Lifecycle ops. `end` writes an **attributed** (Agent · Surface · Machine · Project) fragment via `--summary/--next/--surface/--agent/--project`, folds it in, and surfaces the open-project handoffs + knowledge-harvest still owed; the emitted **session-end skill** carries the judgment checklist (harvest → `knowledge/`, update every `PROJECT.md`, loose ends → `open-threads.md`). | **built ✓** |
| `wsx scan [--json] [--write]` | Detect the user's **own** stack so the interview pre-fills instead of asking cold. Two agent classes: **coding tools** (Claude Code, Cursor, Codex, Gemini-CLI, Copilot, Windsurf, Zed, Aider, Amazon Q, Continue/Cline/Roo/Cody) via PATH + config + macOS `.app` + editor-extension globs → drive files; and **chat/desktop apps** (ChatGPT, Claude desktop, Perplexity, Copilot, Msty, Cherry Studio) → paste a context pack. Also **MCP servers** (names only — never reads keys/env) and **local LLMs** (localhost probes: Ollama/LM Studio/Jan/GPT4All/TextGen). Suggests surfaces/models. **`--find-workspaces`** shallow-walks the usual homes (`~/Documents`, `~/Projects`, an Obsidian iCloud vault, …) for existing workspaces to update — this is what powers the brain's "help me update my workspace" path. Reinforces BYO-tokens — the engine has no API key and makes no model calls. Works with or without a workspace. | **built ✓** |
| `wsx remote [url]` | Decide **where the workspace lives**. No arg → recommends free homes (private GitHub/GitLab/Codeberg, or local-only) + the flow. With a URL → sets the git remote and records `transport.remote`. The person creates their own repo; `wsx` only wires it. | **built ✓** |
| `wsx sync` | Git pull --rebase + push (or point at `wsx remote` when none is configured). The portability backstop. | **built ✓** (basic) |
| `wsx remote <url> --scope personal\|work` · `wsx identity --scope` · `wsx ssh-setup` · `wsx push` · `wsx collab` | **Work/personal GitHub separation, non-overlapping.** `remote --scope` records a remote→scope→identity map in `context/remotes.json` (refuses one identity spanning both scopes, or one URL across scopes). `ssh-setup` appends SSH host-aliases (`github.com`/`github-work`, never overwrites). `identity --scope` applies the mapped **repo-local** name/email (never global, never `-c user.*` — the documented leak vector). `push` finalizes a fresh workspace's auth with the first commit+push, **gated to personal-solo** (work scope refuses; branch→PR→review). `collab` prints (never runs) the `gh` command to add a work account to the private personal repo. | **built ✓** |
| `wsx skill add` \| `list` \| `reindex` | The mechanical hand for the Resolver's **GENERATE** path: create a skill folder + front matter and register it in `manifest.json` (`add`), list skills grouped by hub (`list`), or rebuild the manifest index from disk (`reindex`). `add` takes `--kind hub\|spoke` and **`--level hobbyist\|intermediate\|advanced\|expert` (+ `--seniority`)** — the level sets the skeleton's **altitude** (hobbyist teaches fundamentals + a "Foundations to learn" section; expert assumes fluency + gets "Judgment calls & edge cases" and, with seniority, "Setting the bar & leading"). Writes a **sectioned skeleton** (not a flat stub) whose `Related` block **links its hub + the skills index** (real relative links → Obsidian graph edges); `lint` fails on any generated skill still carrying `_(…)_` prompts. Also regenerates the MOC layer. | **built ✓** |
| `wsx project new <name>` \| `list` \| `adopt <path>` | Per-project **documentation** folders (docs & context, **not** the code/assets). `new` scaffolds `projects/<slug>/PROJECT.md` (overview · where the code lives · live handoff · decisions · pending) + `notes/`. **`adopt`** takes an EXISTING project (git repo or plain folder) **reference-in-place**: detects git remote/branch + stack, writes a regenerated marker-delimited *adoption record*, and points PROJECT.md at where the code lives — a git repo's files are **never copied** into the (public) vault (reference-only, honoring the employer/public wall). `--import-docs` copies a *plain* folder's loose docs into `notes/` (skips secret-like files); `--move` git-aware-relocates the project to sit beside the vault. Idempotent (re-adopt refreshes, no dupes). | **built ✓** |
| `wsx examine [--json]` | **Read-only augmentation readout** for someone who ALREADY has a workspace, so the brain augments additively instead of re-interviewing. Reports: which interview movements (M0–M5) are answered vs still pertinent (→ ask only those), missing scaffold, the hub/spoke/project inventory (→ don't re-propose), and broken connections. The apply step is `wsx upgrade` + targeted `wsx profile set` — nothing pre-existing is removed. | **built ✓** |
| `wsx upgrade [--dry-run]` | Corrective pass over an **already-generated** workspace: **non-destructively** adds any missing scaffold (projects/, sessions/, skill-authoring.md…) and **regenerates the connective MOC layer** (HOME + skills/projects indexes) — the fix that reconnects an islanded Obsidian graph. Never clobbers hand-edited files; idempotent. Applies by default; `--dry-run` previews. | **built ✓** |
...
| `mcp` | A self-contained, zero-dep stdio MCP server (`server.py`) exposing `context_load`, `skills_search`, `skills_load`, `session_start`/`session_end`, `reconcile` over the live workspace, plus a ready-to-paste `mcp.json` client config + README. Portable: stdlib-only, discovers its own workspace root. | `adapters/mcp/` | built ✓ |
...
4. **Emitter (Phase 3).** `wsx emit claude-code` **first** (dogfood the recommended path), then the thin agnostic fallbacks `agents-md` / `cursor` / `pack`. SPEC §7 slotted `mcp` (the universal runtime) second, but as it's heavier it landed **after** the lightweight adapters — which already cover Cursor, Codex, Copilot, and Gemini. `emit mcp` is **now built**: a self-contained zero-dep stdio MCP server. (The status tables in §4–§5 are the honest current state.)
...
- **Privacy** — personal context must not leak into a synced/shared repo. **Answer: personal context is local-only and gitignored**; `personal.private = true` means *never synced* and never emitted into any adapter. **No encryption is implemented or offered** — gitignoring is not encryption; at-rest protection belongs to the OS (FileVault/BitLocker/LUKS). Walled by default, opt-in blend. When in doubt, wall it.
...
*The build is early but real: `init`, `profile`, `emit` (`claude-code`/`agents-md`/`cursor`/`pack`/`all`), `lint`, `verify`, `session`, and `sync` run today and pass a smoke test; `resolve` is built (plan-driven fetch/pin/namespace/overlay + trust gate + composite reference-citing), `search` gives two-track discovery, and `emit mcp` writes a runnable zero-dep MCP server — no command-surface stubs remain; the interview brain is a registered auto-triggering skill (`.claude/skills/bootstrap-gen/`) that drives the loop interactively, not a headless automated loop. When the code and this doc disagree, the code and its `--help` are the truth; this doc is the map.*
