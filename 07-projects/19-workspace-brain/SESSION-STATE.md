...
_Last updated: 2026-09-24 — ZV wave 0: G4e closed; promotion batch landed; re-pin is Sean's next step_
...
- **Parallel thread (2026-09-22): Zero-Vector harness plan v1.1, WAVE 0 APPROVED (LLM-inclusive caveat).**
  ...
  - **Next action:** G4e closed (Sean, 2026-09-24). Promotion batch landed ahead of G5c at Sean's word: claude-code H2/H19 and cursor H19 promoted to enforced-partial, H17/H22 refs moved (H22 names the G5c apply as its remaining condition), codex H19 held back, H17-R7 text updated, claude-code probe re-taken after G4e, a ws_hook probe-merge fix and the prune dedupe. **Sean re-pins next** (`workspace-doctor.sh --install-pin` in a plain terminal at the batch commit); until then `vetted-status prune-our-branches` reads hash-mismatch and the prune skips employer repos from Claude. G5c apply waits for a real merged branch of ours. Personal MBP G3d and G4d remain.
