...
_Last updated: 2026-09-24 — ZV wave 0: D4 = wait for wave 1 on the Personal MBP; only the G5c apply remains_
...
- **Parallel thread (2026-09-22): Zero-Vector harness plan v1.1, WAVE 0 APPROVED (LLM-inclusive caveat).**
  ...
  - **Next action:** Wave 0 closes on the Work MBP; only the G5c apply remains (waits for a real merged branch of ours; do not manufacture one). 2026-09-24 18:22Z: the Subatomic session-end prune ran vetted with zero deletions because PR #20's branch was already deleted at merge (gh pr merge --delete-branch). To close G5c, merge the next PR of ours WITHOUT deleting its branch and let /session-end prune it. **Personal MBP: no Claude overlay until wave 1 (D4, Sean 2026-09-24)** — Codex desktop and Grok Build there apply Claude Code's settings env, so the overlay needs the Claude-only CLAUDE_ENV_FILE channel first (wave 1, with H15). The installer now refuses on git without config hooks (40299c8) and on any Claude settings env seen in another host's shell (ae64ba1). Personal MBP git is 2.55.0 (Homebrew first on PATH via ~/.zprofile). Work MBP doctor drift cleared 2026-09-24 (Cursor shims reinstalled, retired scripts removed, launchd timer loaded, audit MISS acked; `--check` clean outside the sandbox). Watch: re-probe Codex on the Work MBP after Codex updates. Follow-ups done 2026-09-24: per-via env probes landed; the Copilot AI_AGENT mapping is deferred to wave 1 (it would lower the walls; L-01). Work MBP re-pinned to `02e29e1` (install-log entry written, prune vetted, floor passes).
  - **Wave 1 (started 2026-09-24):** spec held at `.claude/state/held/INTENT-wave1-2026-09.md`. Done: W1-a ^pc-47 email scrub (`a32d08a`); W1-b seven wave 0 follow-ups plus a review fix (`68b9835`; **Work MBP re-pin needed**); W1-c H6 measurement (held report; recommended cut set C1+C2+C3+C4+C6+C8 → Codex headroom ~5.2 KiB, awaiting D-W1-1). Everything else waits on Sean's decisions D-W1-1..6 (listed in the spec and the plan note).
