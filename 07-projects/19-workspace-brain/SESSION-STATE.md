...
- **Parallel thread (2026-09-22): Zero-Vector harness plan v1.1, WAVE 0 APPROVED (LLM-inclusive caveat).**
  ...
  - **Next action (agent):** the agent side of wave 0 is done (2026-09-24): all 16 machine gates pass at `d9dcf95` with CI green; Sean's re-verification decisions are implemented. Waiting on Sean's human steps in order: pin install and scan, then G3b probes (which also reinstall the Cursor shim), G3c answer, G4e overlay install, G5c prune with the sandbox off; Personal MBP G3d and G4d in parallel. The agent gives each step's exact commands when it is ready.
