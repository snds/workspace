...
_Last updated: 2026-09-24 — ZV wave 0: G3d/G4d closed; only the G5c apply remains_
...
- **Parallel thread (2026-09-22): Zero-Vector harness plan v1.1, WAVE 0 APPROVED (LLM-inclusive caveat).**
  ...
  - **Next action:** Wave 0: all gates closed except the G5c apply (waits for a real merged branch of ours; do not manufacture one). G3d/G4d closed 2026-09-24 (`567380e`). **Personal MBP git is 2.50.1 with no config-hook support**: do not install the v5 overlay there until git is >= 2.54 and the installer refuses on config_hooks false (held log). Follow-up: map `AI_AGENT=github_copilot_vscode_agent` to copilot-vscode. Doctor drift on the Work MBP remains Sean's.
