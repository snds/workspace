...
- **Parallel thread (2026-09-22): Zero-Vector harness plan v1.1, WAVE 0 APPROVED (LLM-inclusive caveat).**
  ...
  - **Next action (agent):** G3b and G3c are closed (2026-09-24): no host outside Claude Code imports the Claude overlay env, so D4 is not needed. Next is Sean's G4e (scan, then install the v5 overlay, then restart Claude Code); after the restart the agent confirms the floor is registered, then G5c (one vetted prune with the sandbox off), then the post-G4e surfaces.json promotion batch and one re-pin. Personal MBP G3d and G4d remain.
