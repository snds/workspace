...
- **Parallel thread (2026-09-22): Zero-Vector harness plan v1.1, WAVE 0 APPROVED (LLM-inclusive caveat).**
  ...
  - **Wave 1 (started 2026-09-24):** spec held at `.claude/state/held/INTENT-wave1-2026-09.md`. Done: W1-a ^pc-47 email scrub (`a32d08a`); W1-b seven wave 0 follow-ups plus a review fix (`68b9835`; **Work MBP re-pin needed**); W1-c H6 measurement. **H6 (W1-1) done at `94b4b34`**: AGENTS.md cuts, beacons from beacons.json, Windsurf rule, route digest, harness floors. Sean's install steps for H6 are on the checklist (Codex beacon, ~/Projects pointer, paste texts). Next: H18 / H15 / H7 in parallel worktrees. Everything else waits on Sean's decisions D-W1-1..6 (listed in the spec and the plan note).
