...
import { Sparkles, Sun, Moon, FileCode } from "lucide-react";
...
export function ColorsSection() {
  ...
  const clampedPrimary = useMemo(() => {
    ...
    if (brightnessClamp) {
      ...
      const [, C, H] = oklch;
      return oklchToHex(0.55, C, H);
    }

    if (lightnessClamp) {
      const hsl = hexToHsl(brandColors.primary);
      if (!hsl) return null;
      return hslToHex(hsl[0], hsl[1], 50);
    }
  ...
  const toggleTokenFormat = (format: TokenOutputConfig["formats"][number]) => {
    ...
    updateProfile({ tokenOutput: { ...profile.tokenOutput, formats: current } });
  };
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (result) {
        ...
        if (projectId) updateProject(projectId, { colorSystemId: result.id });
      } else {
  ...
  const palettesToShow = colorSystem ? Object.values(colorSystem.palettes) : [];
  const hasDataViz = Boolean(colorSystem?.dataViz?.colors?.length);

  return (
    <>
      {/* ── Brand subsection ───────────────────────────────────────────── */}
      <section id="section-colors-brand" className="scroll-mt-4">
        ...
        <div className="flex gap-6">
          ...
          <div className="w-64 flex-shrink-0 flex flex-col gap-4">
            ...
            <Button
              ...
              disabled={generating || !brandColors.primary || !isValidHex(brandColors.primary)}
          ...
          <div className="flex-1 min-w-0">
            {generated && (
              <div className="flex items-center gap-2 mb-3">
                <Badge
                  variant="outline"
                  className="text-[9px] border-[var(--green-9)] text-[var(--green-11)] bg-transparent"
                >
                  Generated
                </Badge>
                ...
                <div className="flex rounded-lg border border-[var(--mauve-5)] overflow-hidden">
                  <button
                    ...
                    className={cn(
                      "flex items-center justify-center gap-1 px-2 py-1 text-[10px] transition-colors",
                      mode === "light"
            ...
            {!generated ? (
                  ...
                  {["Primary", "Neutral", "Success"].map((label) => (
                    <div key={label} className="flex flex-col gap-1">
                      ...
                      <div className="grid gap-0.5" style={{ gridTemplateColumns: "repeat(12, 1fr)" }}>
                        {RADIX_STEPS.map((s) => (
                          <div key={s} className="bg-[var(--mauve-4)] rounded" style={{ minHeight: 32 }} />
                        ))}
                      </div>
                    </div>
                  ))}
              ...
              <div id="section-colors-scales" className="space-y-3 scroll-mt-4">
                <PaletteScaleHeader />
      ...
      {/* ── Data Viz subsection ────────────────────────────────────────── */}
      {hasDataViz && colorSystem && (
        <section id="section-colors-dataviz" className="scroll-mt-4 mt-8">
          <h3 className="text-[11px] font-semibold text-[var(--mauve-10)] uppercase tracking-wider mb-4">
            Data Visualization
          </h3>
          <DataVizSection
