...
#
# Clones the repo to a temp dir, runs setup.py. Setup then git-clones the
# workspace into your local checkout as part of its normal flow.
...
if [ -z "$REPO_URL" ]; then
  ...
  echo "  export CLAUDE_WORKSPACE_REPO=git@github.com:snds/workspace.git"
  echo "  curl -fsSL https://raw.githubusercontent.com/snds/workspace/main/00-bootstrap/setup/bootstrap.sh | bash"
  exit 1
fi
