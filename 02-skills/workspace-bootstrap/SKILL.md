...
The Claude Workspace is **also a Claude Code working directory and Obsidian
vault** as of 2026-04-23. See `OBSIDIAN-SETUP.md` at the workspace root for
the full three-way integration. When running in Claude Code (not Claude Desktop),
the `.claude/hooks/dispatcher.py` SessionStart hook performs the equivalent of
this bootstrap — this skill remains the authoritative protocol for Claude Desktop.
...
**Skills are NOT loaded at boot.** The system prompt's `<available_skills>`
block already provides skill names and descriptions for routing. Loading
skill files consumes significant context (~145k tokens for all 55+ skills).
Instead, skills are loaded on-demand when the conversation topic requires
them — see Skill Loading Protocol below.

**Manifest reconciliation only:**
Run the reconciliation script once per session after DC is confirmed:
```
start_process("python3 '[workspace root]/02-skills/reconcile-manifest.py'")
```
This keeps the manifest's hash registry current (<1s, silent unless changes
detected). It does NOT load any skill file contents.

---

## Skill Loading Protocol (Token-Optimized)

**Principle: Load the minimum skill set needed for the current question.**
The full skill library is ~145k tokens. A typical question needs 1–3 skills
(5–30k tokens). Never load skills speculatively.

### Routing (zero cost — already in system prompt)

The `<available_skills>` block provides name + description for every skill.
Use these descriptions to match the user's message to relevant skills. This
routing layer costs zero additional tokens — it's already in context.

### Loading tiers

**Tier 0 — Session context (loaded at boot):**
- `session-log.md`, `project-context.md`, `user-preferences.md`
- These are small context files, not skills. Always loaded.

**Tier 1 — Primary skill (loaded on first relevant message):**
- Match the user's message against `<available_skills>` descriptions
- Load exactly 1 skill file (the best match)
- If the skill is a hub (e.g., `variable-icon-font-architect`), load only
  the hub — NOT its spokes. The hub contains a spoke manifest table for
  further routing.

**Tier 2 — Supporting skill (loaded when the conversation needs it):**
- If the primary skill references a spoke or companion skill that's needed
  for the current question, load it now
- Maximum: 1–2 supporting skills per question
- If the topic shifts mid-conversation, load the new skill and note it

**Tier 3 — Deep dive (loaded on explicit drill-down):**
- Math spokes, framework spokes, or reference documents
- Only when the conversation enters that specific domain
- Example: user asks about bezier math → load `math-bezier-spline-theory`

### What NOT to load

- Don't load project context skills (`legion-project`, `omni-project`) unless
  the user explicitly references that project
- Don't load framework spokes (`fw-shadcn`, `fw-vue`, etc.) unless the user
  is working in that framework
- Don't load all spokes of a hub — load the 1–2 relevant to the question
- Don't preload skills "in case they're needed later"

### Hub skill protocol

Hub skills (`variable-icon-font-architect`, `design-engineer`) contain spoke
manifests — tables listing each spoke's domain and trigger conditions. When a
hub is loaded:

1. Read the spoke manifest table in the hub
2. Identify which 1–2 spokes are relevant to the current question
3. Load only those spokes
4. If the conversation shifts to a different spoke's domain, load it then

### When to ask the user

If the opening message is ambiguous (e.g., "let's get started" with no topic):
- Complete the boot sequence
- Ask what they'd like to work on
- Load skills based on their answer

If the message clearly maps to a skill: load it without asking.

### Loading mechanics

Skills live in Google Drive and are read via Filesystem MCP or Desktop
Commander (in priority order):
1. Filesystem MCP: `read_text_file("[workspace root]/02-skills/[name]/SKILL.md")`
2. Desktop Commander: `read_file` at the same path
3. VM mount: `/mnt/skills/user/[name]/SKILL.md` (may be stale)

Prefer Drive sources — they have the most recent versions.
...
Resolve the Drive workspace root for macOS first (see "OS Path Resolution → Step 3"
below — it probes **both** the Mirror and Stream locations and validates that files
are materialized, not online-only placeholders), then immediately attempt to read
`session-log.md` via Desktop Commander. Do not reason about whether this will work —
just do it.

```
Attempt: read <resolved workspace root>/06-context/session-log.md via Desktop Commander
  Success (real file contents returned)  → DC mode confirmed. Proceed to Step 2A.
  Empty / 0-byte / placeholder returned  → mount is dehydrated. Try the NEXT candidate
                                           root, or force-hydrate via DC read, then retry.
  Failure (actual tool error)            → DC unavailable. Proceed to Step 2B.
```

**Important (post-2026-06-01):** the resolved root may be either
`~/My Drive/Claude Workspace` (Mirror mode, current) or the older
`~/Library/CloudStorage/GoogleDrive-.../My Drive/Claude Workspace` (Stream mode).
Do not assume one. If the first candidate returns empty/placeholder content, that is
**not** a DC failure — it's a stale/dehydrated mount; move to the next candidate
before falling through to Step 2B.
...
[workspace root]/06-context/relational-context.md
```

`session-log.md` is already loaded from Step 1.

`relational-context.md` is the personal/relational thread (Claude-authored).
It carries the human texture of the working relationship, not project state.
Read it at boot so the collaboration's continuity — not just the deliverables —
persists across sessions. If missing, proceed without it; it is created the
first time the user asks to record a personal/relational note. Treat absence
as normal, not an error.

CLAUDE.md is the canonical source for the mandatory session-start ritual format (see Step 3) and the workspace conventions. Reading it here ensures parity with Claude Code, where the same ritual is mandated via the SessionStart hook.
...
**Framework layer awareness.** The workspace contains five top-level operating
frameworks at `[workspace root]/00-frameworks/`. These are not loaded at boot
(token cost is too high and most sessions don't need all five), but skills and
direct conversation can reference them on demand. At boot, note whether the
frameworks folder exists and log it silently — absence is a flag that the
workspace may need re-sync. For orientation, see `00-frameworks/00-README.md`.

**Per-project operational state (SESSION-STATE.md).** If the session-log or the
user's opening message indicates an active project, attempt to read its
operational state file:

```
Attempt: read [workspace root]/07-projects/[active-project]/SESSION-STATE.md
  Success → parse Current state + most recent session history entry
  Missing → proceed without it (project hasn't been seeded)
  Error   → proceed without it (log but don't block)
```

When SESSION-STATE.md is found, surface in the boot confirmation:
- Machine and branch from Current state.
- "Needed to resume" from the most recent history entry.
- Any pending questions flagged in the state file.

Example addition to boot confirmation:
```
✓ Workspace loaded [DC] — resuming 14-variable-icon-font-generator
   State: Voyager-2.local on branch `feature/masters-v0.3`, uncommitted wght-axis work
   Needed to resume: decide on GRAD axis derivation approach
```

During the session, append silent rolling updates to SESSION-STATE.md's
"Current state" section when operational conditions change (dev server
started, branch switched, MCP bridge connected/disconnected, config changed).
These updates don't create session history entries — only explicit checkpoints
and wrap-ups do. See `00-frameworks/_session-state-template.md` for the full
spec.

If Step 0 was deferred, run the context survey + manifest reconciliation now.
Skills are NOT loaded here — they load on-demand per the Skill Loading Protocol.
...
Render the **mandatory session-start ritual** per CLAUDE.md's "Session-start ritual (mandatory)" section. CLAUDE.md is loaded in Step 2A, so its format spec is in context — render it exactly as specified there.

The ritual format is the canonical confirmation. Same shape across all surfaces (Claude Code, Claude Chat with DC, Cowork) so Sean sees consistent output regardless of where he's working.

**Surface field for this skill:** identify as one of:
- "Claude Desktop (chat) + Desktop Commander" — DC mode
- "Claude Desktop (chat) + Drive MCP fallback" — Drive MCP mode
- "Claude Desktop (chat) + embedded snapshot — context may be stale" — snapshot mode

These go into the ritual's **Surface** line. The ⚠ snapshot variant should also appear as a Notice at the top of the ritual (per CLAUDE.md's notice-rendering rule).

**Boot mode notes** (incorporated into the ritual where relevant — typically as appended detail on the "Workspace loaded" line or as additional notices):
- Skills initialization ran: `— [N] skills initialized`
- Cowork sync ran: `— [N] skills synced` (or omit if all were already current)

Then proceed directly into whatever the user needs. Do not list docs read. Do not narrate the process.
...
Google Drive for Desktop has **two sync modes** that put the workspace in
**different locations**, and the location can change if Sean switches modes:

| Mode | Workspace root | File state on disk |
|---|---|---|
| **Stream** | `~/Library/CloudStorage/GoogleDrive-hello@snds.design/My Drive/Claude Workspace` | Online-only by default — files may be **references, not local bytes** (0-byte placeholders until hydrated) |
| **Mirror** | `~/My Drive/Claude Workspace` | Fully downloaded — real local files |

**Resolve by probing candidates in order and validating materialization.** A path
is only valid if it both exists *and* contains a **real, non-empty file** — not just
an online-only placeholder. Checking existence alone is the trap that breaks boot
after a mode switch (a stale stream mount can still list directories while every
`SKILL.md` is a 0-byte reference).

```bash
# Canonical probe file used to prove the mount is real + materialized.
PROBE="06-context/session-log.md"

resolve_workspace_root() {
  for ROOT in \
    "$HOME/My Drive/Claude Workspace" \
    "$HOME/Library/CloudStorage/GoogleDrive-hello@snds.design/My Drive/Claude Workspace" \
    $HOME/Library/CloudStorage/GoogleDrive-*/My\ Drive/Claude\ Workspace ; do
    # Must exist AND the probe file must have real local bytes (size > 0).
    if [ -d "$ROOT" ] && [ -s "$ROOT/$PROBE" ]; then
      echo "$ROOT"; return 0
    fi
  done
  return 1
...
- **Mirror is listed first** (current mode as of 2026-06-01) so a fully-downloaded
  mount wins immediately.
- `[ -s ]` (size > 0) is the materialization gate. If a candidate exists but the probe
  file is 0 bytes, that mount is **dehydrated** — skip it and try the next candidate.
- If multiple Drive accounts exist under `CloudStorage/`, the glob prefers
  `GoogleDrive-hello@snds.design`; the wildcard is the fallback.

**If the only candidate that *exists* is dehydrated** (probe file is 0 bytes / all
`SKILL.md` are placeholders), do not proceed silently against placeholders. Either:
1. Force-hydrate via Desktop Commander `read_file` on the needed files (DC reads pull
   bytes down from Drive), then re-validate `[ -s ]`; or
2. If hydration isn't viable, note it and fall back to **Step 2B (Drive MCP)**:
   `⚠ Google Drive workspace is online-only (not materialized) — falling back to Drive MCP.`

> **Mode-switch note.** Stream→Mirror relocates the root from `CloudStorage/...` to
> `~/My Drive/...` *and* forces a full download. Mirror→Stream does the reverse and may
> leave files online-only. Always re-resolve at boot; never hardcode one path.

##### Per-machine sync modes (automated via SessionStart hook)

Sean runs **different Drive modes per machine on purpose**: Stream + *Available
offline* on the work Mac, Mirror on the personal MacBook and Windows desktop. Because
`~/.claude/` is **machine-local (not Drive-synced)**, each machine carries its own
`~/.claude/skills` symlink pointing at its own materialized root.

This is kept correct automatically by a SessionStart hook:
`~/.claude/hooks/resolve-skills-symlink.sh` (canonical copy + install note at
`00-bootstrap/setup/resolve-skills-symlink.sh` and `INSTALL-skills-resolver.md`). It
applies the same first-materialized-candidate rule as `resolve_workspace_root()` above
and repoints the symlink when needed — idempotent, non-destructive, non-fatal if Drive
is offline.

| Machine | Drive mode | Resolved skills root |
|---|---|---|
| Work MacBook | Stream + offline | `~/Library/CloudStorage/GoogleDrive-…/My Drive/Claude Workspace/02-skills` |
| Personal MacBook | Mirror | `~/My Drive/Claude Workspace/02-skills` |
| Windows Desktop | Mirror | `G:\My Drive\Claude Workspace\02-skills` |

At boot, the symlink can therefore be trusted on any machine — but the
`resolve_workspace_root()` probe remains the authority for the *workspace root* used by
the rest of this skill (artifacts, context, session log), since those paths are
addressed directly, not via the skills symlink.
...
**Write 5 — wrap up SESSION-STATE.md for the active project (if any):**
```
File: [workspace root]/07-projects/[active-project]/SESSION-STATE.md
Action:
  - Rewrite the "Current state" section atomically with final values.
  - Append a new session history entry at the top of that section:
    * Type: wrap-up
    * Focus: one sentence on what the work was
    * Machine: resolved hostname
    * Accomplishments: bullets of key things done
    * Decisions: only significant ones (not covered by project docs)
    * Next resumption needs: what future-Sean needs to pick up
If no active project was worked on this session, skip this write.
```
...
```
Claude Workspace/
├── 00-frameworks/               ← top-level operating frameworks (READ-ONLY at boot)
│   ├── 00-README.md             ← orientation + compressed summaries of all five
...
│   ├── team-practices-and-decisions.md
│   └── _session-state-template.md
├── 02-skills/
│   ├── skills-manifest.json     ← hash registry + sync metadata
...
│   ├── relational-context.md    ← personal/relational thread (writable, Claude-authored)
│   └── artifact-registry.md    ← structural file index (auto-maintained)
├── 07-projects/
│   └── [project]/SESSION-STATE.md  ← per-project operational state (writable)
├── 04-artifacts/
