...
**Auto-loaded by:** drift-detection, figma-variable-audit, schema-validator, token-audit, token-compliance, token-documentation

---

## The three-tier model

Design tokens are most useful when they are structured in three tiers, each serving a distinct purpose. Collapsing tiers — using only primitives, or skipping primitives entirely in favour of semantics — creates problems that compound over time.
...
Component tokens scope semantic intent to a specific component context. They reference semantic tokens and map intent to a specific application.
