...
H23 adds the touch ledger and the sweep entry. `--event post-tool` appends `{repo, path, tool, ts, host}` lines to `~/.config/snds-workspace/telemetry/sessions/<sid>.touched` from Claude PostToolUse (edits and Bash), Cursor afterFileEdit and afterShellExecution, and Codex PostToolUse (apply_patch and Bash); a shell command records its repo with no path. It never imports profile_resolve, never prints, fails open, and writes nothing until `telemetry/` exists. `sweep --host H` (a session-start registration on every hooked host) runs the pinned sibling `closure.py` sweeper within its budget and prints only the host's noop.

## closure.py

Session closure per touched repo (H23) and H25's fragment limits. `plan --session SID|latest [--json] [--family F]` reads the touch ledger (no ledger: `git status` over the workspace and the checkout cache; a Claude session reads only the workspace and positively personal repos), classifies each repo through `profile_resolve`, and prints one action per repo: `workspace` (fold, rebuild, verify, commit, push under the resolved identity), `commit-push` or `hold` (positively personal), `handoff` (not positively personal, from Claude: a note and vetted housekeeping only, never git) or `branch-pr` (from Cursor or Codex: feature branch and PR, never the default branch). `ws closure …` is the neutral entry. `fragment --session SID PATH` applies the H25 limits in place: a fragment from a session that touched a repo that is not positively personal keeps only `{slug, status, PR URL if allowed}` for it (an opaque id and no URL in this public vault); `compact-sessions.py` applies the same limit at fold time. `sweep [--dry-run] [--json]` is the session-start sweeper by hand: for each dead session (no ledger, claim or baseline activity for 4 h) it commits only the mechanical workspace leftovers no live session claims (its `06-context/sessions` fragment and the fold and rebuild outputs), under the resolved identity, and never pushes; substantive leftovers become one notice in `telemetry/closure-notices.json`, which `session-status.py` shows on the card. `--self-test` runs `fixtures/closure/cases.py`; TestClosure runs the same cases.
