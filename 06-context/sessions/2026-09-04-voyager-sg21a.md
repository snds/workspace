...
Migration done this session (vault content → correct homes):
  ...
  - 12-MCS: BLOCKED — folder is empty (0 files; only empty cpes-software + saas-plm-analysis/knowledge-discovery scaffold dirs). No research to relocate; no snds/saas-plm-analysis repo found on GH. Needs Sean's guidance (where the real research lives, or remove empty scaffold).
Next:
  - Phase 1 per DESIGN-PLAN §10 — begin graph MVP in ~/Projects/ShadeGraph
  - Resolve 12-MCS (empty scaffold) — locate the actual saas-plm-analysis research/repo or remove the stub.
  ...
  - Create GH remote for ShadeGraph if desired (currently local-only at ~/Projects/ShadeGraph).
--- END BLOCK ---
