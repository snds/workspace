...
- [ ] **Employer design-system migration (cpes-software)..** (Added 2026-07-15; **unblocked 2026-07-20**.) **Inventory re-run 2026-08-11** in employer repo: `MIGRATION-TO-CENTRIC-UI.md` + gap map + `CENTRIC-UI-SYNC.md` + `DESIGN-SYSTEM.md` rewritten for **bridge-then-consume** — product UX still originates in the proto; DS write surface = centric-ui; end state = proto depends on `@centric/ui` + tokens + `@centric/data-table` (Tracks L → P → C). Baselines: proto `feat/interaction-overlay-tokens@42f8ba1`, cui #284 `98e5ca66` (density/interaction/landing/StatusPill — **not on main@f2efaad7**). Jul-20 reuse-and-extend screen spine still valid; new units cover density, interaction overlays, proto-only primitives, packaging. Deliverables stay in the **employer repo** (separation rule). Next: merge #284; Track L lifts (chip-multi-select, accordion/context-menu/slider/toggle-group, caution #87); Track P package extract; Track C proto consume. **🔄 2026-08-12 Sean:** Tracks L/P/C largely landed (cui #284/#288, proto #20–#45). **Do not delete proto screens** because centric-ui already has RecordPage / sidebar / media dialogs — proto is Olga+Sean's design sandbox; keep the pages. Consume DS leaves; lift *new* components/compositions into centric-ui. See [[decision-proto-is-design-sandbox]]. **🔄 2026-09-15:** CDS `Icon` (Material Symbols) + proto consume are on main (cds #39/#40, proto #80). Next: cui tickets to consume `@centric/ui/icon`. **🔄 2026-09-18:** When cui starts host federalization, re-align ButtonGroup / split CTAs from [[cds-host-buttongroup-realign]] — live cui has no ButtonGroup (dark single CTA); older split was dark not primary/outline; proto uses `SplitActionButton`. Do not copy CDS Default filled primary into toolbars.
