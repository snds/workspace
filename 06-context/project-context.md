...
- [ ] **Paste the workspace beacon into the chat surfaces — SPLIT per Sean 2026-07-09:** (a) **Personal MBP, now:** claude.ai preferences + claude.ai "Workspace" project (+ GitHub sync) — `pbcopy < 00-bootstrap/dist/BEACON.md`, paste, then `workspace-doctor.sh --ack-chat` (ack state is per-machine, so acking here keeps the Work MBP nag alive). (b) **Work MBP, with the doctor install there:** Cursor User Rules + Perplexity Space — those tools live/are used there; its own doctor nags until acked on that machine. Added 2026-07-06; split 2026-07-09.
