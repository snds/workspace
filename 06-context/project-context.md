...
**Next:** Colleague/Olga re-test of full v0.2 (bridge point auto-orient, restructure on a real flat vault, ingest on a real notes folder). Follow-ups: bump `__version__` so `diagnose`'s stale-`.wsx`-copy check has a signal; optionally have `health` scan wired `wire` extras. **Discipline:** test CLI changes on a FRESH `init` (or after `wsx upgrade`) — never a reused workspace (stale `.wsx` copy masks fixes/regressions). Zips regen via `package.py` (gitignored).
