...
- [ ] **Finalize the figma-repo-sync-plugin against the now-conformant Figma library (2026-06-20).** The transliterated centric-ui Figma library is finished & fully conformant: **mode-first** architecture (intent/tone/size = variable modes, presence = booleans, physical variants only for true structure), **~100% token coverage** (incl. zeros/blanks → `space-0`/`radius-none`/`border-width-0`/new `transparent`), surface conventions, clean structure. New durable laws codified in framework **§8e** (mode-first + total tokenization) + **06 §6** (adversarial verification) + the figma memories. Next-step playbook + checklist in [07-projects/09-figma-repo-sync-plugin/finalization-synthesis.md](../07-projects/09-figma-repo-sync-plugin/finalization-synthesis.md): switch foundation identity to `setSharedPluginData`; expose `scaffoldFoundations()` CLI; generator to emit mode-first + total-tokenization by construction + an unbound-value lint gate; land the codebase-punch-list deltas; add Code Connect.
