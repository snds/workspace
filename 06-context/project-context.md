...
- [ ] **Push `feat/radix-color-system` + open/refresh the figma-repo-sync-plugin PR.** Unpushed commits: `2f88837` (plugin Build 11.4.10), `5ed5274` (caution Badge/Button variants), `7d9dc15` (cds-* → semantic codemod), + the earlier Radix color-system work. Company repo — team review, **no self-merge**. After merge, regen the Figma library.
...
- [x] ~~**Kickoff Stream A + Stream C of the Mobbin enterprise-SaaS pattern audit.**~~ Done 2026-05-12 — Stream A (20 PLM-critical) fast pass + deep re-audit of 6 conditionals complete (5/6 gaps closed). Stream C (8 AI-forward) fast pass complete. 28/28 patterns graduate. Synthesis at `04-artifacts/active/enterprise-saas-pattern-synthesis_v0.1_2026-05-12.md`. Master operational entry shipped at `08-knowledge/design/enterprise-saas-design-patterns.md`, registered in `_INDEX.md` with 22 trigger words; cross-linked from `ds-advisor` and `design-engineer` SKILL.md.
...
- [ ] **Populate `team-practices-and-decisions.md`.** Fills in passively as decisions surface; not a discrete task. Scaffold lives at `00-frameworks/team-practices-and-decisions.md`.
...
- [x] ~~Clean up duplicate skill files at `02-skills/` root.~~ Done 2026-04-27 — 5 strict duplicates archived; 2 (figma-ds-generation-pipeline, figma-style-binding) had unique content merged into their dir SKILL.md before archive. See `06-context/audit-log.md`.
...
- **2026-04-21 (Framework layer):** `00-frameworks/` folder with five framework docs + README + team-practices scaffold + session-state template. `workspace-bootstrap` extended with framework awareness + SESSION-STATE.md loading + Write 5 at session end. Opus 4.7+ skill audit report delivered.
...
On 2026-04-21 a framework layer was added to the workspace. Five top-level operating frameworks now live at `00-frameworks/` in the workspace root:
...
Orientation + compressed summaries at `00-frameworks/00-README.md`. These frameworks sit above any project-specific skill or context. They inform design, collaboration, research, and craft decisions across every project in the workspace.
...
A skill audit report (`04-artifacts/active/skill-network-audit_opus-4.7_2026-04-21.md`) identified five prioritized opportunities for aligning the skill network with the new framework layer. None of the recommendations have been executed yet — the report is a deliverable, not an action log.
