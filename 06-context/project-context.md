...
- [ ] **Run the workspace fix session (FX-1..FX-14).** Prompt: `05-artifacts/active/workspace_fix-session-prompt_v1.0_2026-07-09.md` (machine-local, Personal MBP); source: `workspace_validation-report_v1.0_2026-07-09.md`. Launch from the workspace ROOT; Phase A (machine-layer doctor install) first; ends with a validation-harness re-run to compare scorecards. Headliners: dist v2 install (P0-4), dispatcher emit-order/dedupe (P0-2), audit-class trigger for #06 (P0-3), SESSION-BLOCK-aware last-session parser (P1-5), Proofboard amendments (Sean-approved). Added 2026-07-09 (validation session).
...
- [ ] **Paste the workspace beacon into the 4 chat surfaces** (claude.ai preferences · claude.ai "Workspace" project + GitHub sync · Cursor User Rules · Perplexity Space): `pbcopy < 00-bootstrap/dist/BEACON.md`, then `00-bootstrap/doctor/workspace-doctor.sh --ack-chat`. The doctor nags every 4h until acked (by design). Added 2026-07-06 (bootstrap v2 — see memory `decision-bootstrap-v2-guarantee`).
- [ ] **Curate `00-bootstrap/dist/beacon-repos.txt`** — doctor flagged `~/Projects/design-system` and `~/Projects/open-design` as unlisted personal-repo candidates for cloud-session beacon coverage. Never list employer repos. Added 2026-07-06.
