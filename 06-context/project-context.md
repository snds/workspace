...
- [ ] **centric-ui PR #179 — assign reviewers + settle the redirect-URI question..** (Added 2026-07-20.) Conflicts resolved 2026-08-10; MERGEABLE — still needs reviewer (Alex). Detail → [project-context-detail.md#pc-06](project-context-detail.md#pc-06). ^pc-06
...
- [ ] **Zero-Vector harness plan v1.1 (LLM- and device-inclusive): implement wave 0.** (Added 2026-09-22; wave 0 approved with the LLM-inclusive caveat.) Plan v1.1 has 25 components in 4 waves, one mechanism plus generated shims per surface. The identity overlay v2 fix has landed (`0d19852`). Next: the H16/H19/H24 probes and pinned installers, then H2 and H22 (action policy, vetted prune with receipts), then the rest of H17. Decisions still open are listed in the plan note. Plan → [notes/zero-vector-harness-plan_2026-09-22.md](../07-projects/19-workspace-brain/notes/zero-vector-harness-plan_2026-09-22.md); full detail is held locally in `.claude/state/held/` (Work MBP). ^pc-45
...
- [ ] **▶ URGENT: `snds/workspace` is PUBLIC.** (Verified 2026-09-22 via the unauthenticated GitHub API: `visibility: public`, created 2026-06-16.) The vault calls itself private (project-registry.md:19), yet it tracks employer-derived context and names employer repos and PRs. Sean decides: (a) keep it public, scrub going forward and gate new employer substance (H25); (b) make it private (GitHub → Settings → Danger zone; rulesets then need a paid plan); (c) additionally rewrite history (destructive; Sean only). Until he decides, the v1.1 detail and surface research stay local and uncommitted (`.claude/state/held/`). ^pc-47
...
- **PlanetCompiler / independent planet experiment (2026-09-12):** phase-four regional terrain hierarchy and shared native collision are verified at `e9593d3`. Debug/Release each 9 core suites and 105 independent terrain checks, native 24/24, live 1,837/1,837 with 13,200 real collision probes, eight actual controls and twelve independently reviewed captures pass. Source spacing stays 500 m at 32 km/N64; human, geological and production fidelity acceptance remain open. Next: complete the authorized phase-five camera-driven selection and measured-readiness increment. Live state → [independent planet handoff](../07-projects/13-legion/docs/planet-lab-independent/SESSION-STATE.md). Implementation stays in separate personal PlanetCompiler; preserve Legion. Phase five is now authorized and active: camera-driven regional selection with measured committed readiness; frozen contract `9edd216` and isolated G5/U5/V5 worktrees. See live handoff; no phase-five acceptance yet.
