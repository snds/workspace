...
- [ ] **SSH to github.com:22 timing out on the Centric laptop.** (Added 2026-07-20.) Persisted all session; every push went over HTTPS instead. Likely corporate egress filtering. Fix if it recurs: point `Host github.com` and `Host github-work` at `ssh.github.com` port 443 in `~/.ssh/config`. **2026-09-22:** it recurred, and port 443 also timed out while HTTPS worked. Claude now uses HTTPS for `snds/*` (overlay v4). Cursor and Codex still use SSH over `github-work`. ^pc-09
