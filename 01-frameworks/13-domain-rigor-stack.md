...
| Workspace ops (session / audit workflows) | #08 + AGENTS.md handoff protocol | `optimize`, `session-end` (command hubs; one `03-skills` home, generated wrappers per surface) | `close-out-dispatch.py` rows + its `--from-diff` DIFF_CLASSES (H10) + `build-local-skill-plugin.py --check` | cross-cutting, no foundation (by design) | `self-improve` heals; doctrine precedence |
