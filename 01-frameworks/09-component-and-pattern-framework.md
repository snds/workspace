...
- **The three-tier token model** (Spectrum's global / alias / component; Curtis's Options → Decisions): **Global/primitive** (raw `blue-600`) → **Semantic/alias** (`color-action-primary`, carries intent) → **Component** (`button-bg-primary`, local decision). Rule: only use a global token when no alias exists; start a token *inside* a component and **promote** it outward only after repeated reuse. Tokens are DTCG-shaped JSON (§11) so they round-trip to Figma variables, Tailwind, and `DESIGN.md`. **Counter-stance (Frost, *Subatomic*):** tier 2 does the heavy lifting and a component token must *earn* its place — heavily-variable components that differ across themes (buttons, links), component categories (one input border for all form controls), or special cases (focus ring, highlighted row) — the workspace default for multi-theme systems; see [[design-token-architecture]] (disagreement table) · procedure [[token-architecture]] · detector `09-tools/token-audit.py`.
